...
import { transcriptOne, transcriptTwo, transcriptThree } from '../constants/test-data/expectedTranscripts';
...
describe('data ingestion unit tests', () => {
  ...
  it('reads the contents of a file', async () => {
    ...
    const actual = await readRawFile('2016-03-23-001.md', `${__dirname}/test-markdown`);
